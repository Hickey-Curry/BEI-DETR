...
from .LSTM import build_LSTM_EYE
from .EEGNet import build_EEG

import numpy as np

class DETR(nn.Module):
    """ This is the DETR module that performs object detection """
    def __init__(self, backbone, backbone1, eegNet, transformer, num_classes, num_queries, aux_loss=False):
        ...
        self.input_proj2 = nn.Conv2d(backbone1.num_channels, hidden_dim, kernel_size=1)
        self.aux_loss = aux_loss  # True
        # self.vit = vit
        self.eegNet = eegNet
        self.input_proj_vit = nn.Conv2d(768, hidden_dim, kernel_size=1)
    ...
    def forward(self, samples: NestedTensor, samples1: NestedTensor, samples2: NestedTensor):
        ...
        # features1, pos1 = self.backbone(samples)
        features2, pos2 = self.backbone1(samples2)

        features1, pos1 = self.eegNet(samples1)

        # features2, pos2 = self.vit(samples2)
        # len1 = len(pos[0][0])
        # len2 = len(pos[0][0][0])
        # len3 = len(pos[0][0][0][0])
        # pos = nn.Parameter(torch.zeros(1,len1, len2*len3 + 4))
        # pos1 = nn.Parameter(torch.zeros(1,len1, len2 * len3 + 4))
        # pos2 = nn.Parameter(torch.zeros(1, len1,len2 * len3 + 4))
        ...
        # hs: [6, bs, 100, 256]
        hs1, hs2 = self.transformer(self.input_proj(src), src1, self.input_proj2(src2), \
                                    mask, self.query_embed.weight, pos[-1], \
                                    mask1,self.query_embed1.weight, pos1[-1],\
                                    mask2,self.query_embed2.weight, pos2[-1])
        # hs1 = hs1[0]

        # 分类 [6个decoder, bs, 100, 256] -> [6, bs, 100, 92(类别)]
        outputs_class = self.class_embed(hs1)
        # 回归 [6个decoder, bs, 100, 256] -> [6, bs, 100, 4]
        outputs_coord = self.bbox_embed(hs2).sigmoid()
...
def build(args):
    ...
    # vit = build_vit(args, has_logits=False)
    # lstm = build_LSTM_EYE(args)
    eegNet = build_EEG(args)
