...
def build(image_set, args, dim=0):
    """
    ...
    if dim == 0:
        ...
        PATHS = {
            ...
            "test": (root / "test2017", root / "annotations" / f'test.json'),
        }
    elif dim == 1:
        mode = 'instances'
        PATHS = {
            "train": (root / "train-EEG", root / "annotations" / f'{mode}_train2017.json',root/"train-EYE"),
            "val": (root / "val-EEG", root / "annotations" / f'{mode}_val2017.json'),
        }
    else:
        mode = 'instances'
        PATHS = {
            "train": (root / "train-EYE", root / "annotations" / f'{mode}_train2017.json',root/"train-EYE"),
            "val": (root / "val-EYE", root / "annotations" / f'{mode}_val2017.json'),
        }
